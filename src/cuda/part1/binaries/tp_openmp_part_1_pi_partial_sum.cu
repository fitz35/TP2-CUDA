...
__global__ void pi_kernel(float *sum, float step, int steps_per_thread, int num_steps, int num_thread_per_bloc)
{
    float x;
    int i;
    ...
    if (threadIdx.x == 0) {
        float total_sum = 0;
        for (i = 0; i < num_thread_per_bloc ; i++) {
            //printf("partial_sum[%d] = %f \n", i, partial_sum[i]);
            total_sum += prox_partial_sum[i];
        }
        //printf("total_sum = %f \n", total_sum);
        atomicAdd(sum, total_sum);
    }
}
...
int main (int argc, char** argv)
{
      ...
      gettimeofday( &begin, NULL );
        // Launch kernel
        pi_kernel<<<num_cores, num_threads_per_bloc>>>(d_sum, step, steps_per_thread, num_steps, num_threads_per_bloc);
    ...
    cudaFree(d_sum);
      ...
      myfile << "Partial_Sum,"<< num_steps << "," << num_cores << "," << num_threads_per_bloc << "," << time << "," << pi<<std::endl ;
      myfile.close();
      return 0;
}
